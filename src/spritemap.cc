...
void SpriteMap::draw_ex(Graphics& graphics, int n, int x, int y, bool flip, float angle, int cx, int cy) {
  SDL_Rect source = { 0, 0, width_, height_ };
  SDL_Rect dest = { x, y, width_, height_ };
  ...
  graphics.blit_ex(file_, &source, &dest, angle, &center, flip ? Graphics::HORIZONTAL : Graphics::NONE);
}
